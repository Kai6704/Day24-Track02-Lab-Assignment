...
class SimpleVault:
    ...
    def encrypt_data(self, plaintext: str) -> dict:
        ...
        nonce = os.urandom(12)
        # Chuyển plaintext string sang bytes trước khi encrypt
    ...
    def decrypt_data(self, encrypted_payload: dict) -> str:
        ...
        # Giải mã để lấy lại plaintext DEK
        plaintext_dek = self.decrypt_dek(encrypted_dek)
        
        # Tách 12 bytes đầu làm nonce, phần còn lại là ciphertext
    ...
    def encrypt_column(self, df: pd.DataFrame, column: str) -> pd.DataFrame:
        ...
        df[column] = df[column].apply(
            lambda x: json.dumps(self.encrypt_data(str(x))) if pd.notna(x) else x
        )
        return df